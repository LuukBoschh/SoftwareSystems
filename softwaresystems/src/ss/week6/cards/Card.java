...
public class Card
{
	...
	public static void main(String[] args) throws IOException {
		DataOutputStream dataOut = new DataOutputStream(new FileOutputStream("" + "card.dat"));
		Card DJ = new Card(DIAMONDS, JACK);
		DJ.write(dataOut);
		DataInputStream dataIn = new DataInputStream(new FileInputStream("" + "card.dat"));
		Card kaart  = read(dataIn);
    ...
    */
	...
	public static Card read(BufferedReader in) throws IOException{
		String kaartString;
		try {
			while((kaartString = in.readLine()) != null) {
				String[] arr = kaartString.split(" ");
				if(isValidSuit(suitString2Char(arr[0])) || isValidRank(rankString2Char(arr[1]))) {
					Card kaart = new Card(suitString2Char(arr[0]), rankString2Char(arr[1]));
					return kaart;
				}
